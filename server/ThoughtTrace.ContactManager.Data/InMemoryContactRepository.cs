...
namespace ThoughTrace.ContactManager.Data
{
    public class InMemoryContactRepository : IContactRepository
    {
        ...
        public List<Contact> SearchContacts(string value)
        {
            if (!string.IsNullOrEmpty(value))
            {
                return _contacts.Values.Where(w => w.FirstName.ToUpper().Contains(value.ToUpper())
                || w.LastName.ToUpper().Contains(value.ToUpper())
                || w.Email.ToUpper().Contains(value.ToUpper())).OrderBy(x => x.FirstName).ToList();
            } else
            {
                return GetContacts();
            }
